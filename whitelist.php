...
$return_url = ($from === 'readonly') ? 'cyberwebeyeosblacklist.php' : 'cyberwebeyeosblacklistadmin.php';
$return_text = ($from === 'readonly') ? 'Blacklist Görünümüne Dön' : 'Admin Paneline Dön';

// Eger message tanimli degilse, baslangi�ta bos bir deger atayin
...
function display_whitelist($search_ip = '', $per_page = 10, $page = 1) {
    ...
    if ($search_ip) {
        ...
        foreach ($whitelist_items as $item) {
            // Direkt eslesme veya subnet kontrol�
    ...
    // Sayfa basina g�sterim se�enegi
...
function validate_cidr($cidr) {
    if (preg_match('/^(\d{1,3}\.){3}\d{1,3}\/\d+$/', $cidr)) {
        ...
        if (filter_var($ip, FILTER_VALIDATE_IP)) {
            $prefix = (int)$prefix;
            // IPv4 i�in prefix 0-32 arasinda olmali
...
// Bir IP'nin subnet i�inde olup olmadigini kontrol eder
function ip_in_subnet($ip, $subnet) {
    ...
    // CIDR notasyonu i�in kontrol yapalim
    list($subnet_ip, $subnet_bits) = explode('/', $subnet);
    
    // IP adreslerini 32-bit tamsayilara d�n�st�r
    $ip_long = ip2long($ip);
    $subnet_long = ip2long($subnet_ip);
    
    if ($ip_long === false || $subnet_long === false) {
        return false; // Ge�ersiz IP adresi
    }
    
    // Subnet maskesini hesapla
    $mask = -1 << (32 - (int)$subnet_bits);
    
    // IP'nin subnet i�inde olup olmadigini kontrol et
...
<head>
    ...
    <title>Cyberwebeyeos Whitelist Y&ouml;netim Aray&uuml;z&uuml;</title>
...
<body>
    <header class="header">
        <div class="header-content">
            <h1 class="header-title">Cyberwebeyeos Whitelist Y&ouml;netim Aray&uuml;z&uuml;</h1>
            <div class="header-actions">
                <a href="<?php echo htmlspecialchars($return_url); ?>" class="btn btn-primary">
                    <i class="fas fa-arrow-left"></i> <?php echo htmlspecialchars($return_text); ?>
                </a>
            </div>
    ...
    <div class="container">
        ...
        <?php /* 2026-05-14 Whitelist Kaynak Yönetimi bölümü — ana tabloyu bozmaz */ ?>
        <?php @include __DIR__ . '/../_shared/whitelist_sources.inc.php'; ?>
    </div>

    <footer class="footer">
        <p>&copy; 2024 Cyberwebeyeos. T&uuml;m haklar&#305; sakl&#305;d&#305;r.</p>
    </footer>
</body>
</html>